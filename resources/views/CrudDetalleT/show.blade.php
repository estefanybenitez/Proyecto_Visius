...
@section('tabla')
    ...
    <h5 class="text-center p-2 ">Detalle Tarea</h5>
    <hr>
    ...
    {{-- <table class="container table table-hover table-bordered mt-2"> --}}
        {{-- Encabezados --}}
                    ...
                    <tr>
                        ...
                        <a class="btn btn-primary btn-sm" href="/products/edit/{{$item->id_detalle}}">Modificar</a>
                        {{-- boton para eliminar --}}
                        <button class="btn btn-danger btn-sm" url="/products/destroy/{{$item->id_detalle}}" onclick="destroy(tdis)" token="{{csrf_token()}}">Eliminar</button>
                        </td>
                    </tr>
                    @endforeach
                    </table>
